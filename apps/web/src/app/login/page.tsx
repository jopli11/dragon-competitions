...
function LoginContent() {
  ...
  const handleGoogleLogin = async () => {
    if (!auth) {
      console.error("Firebase Auth not initialized. auth object is:", auth);
  ...
  return (
    <div className="bg-white rounded-[2.5rem] border border-brand-primary/5 shadow-2xl p-8 sm:p-12">
      <div className="text-center mb-10">
        <BrandSectionHeading>Welcome <GradientText>Back</GradientText></BrandSectionHeading>
        <p className="mt-2 text-brand-midnight/40 font-bold uppercase tracking-widest text-[10px]">
          Login to your Coast account
        </p>
      </div>
      ...
      <form onSubmit={handleLogin} className="space-y-6">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between ml-4">
            <label htmlFor="password" className="block text-[10px] font-black uppercase tracking-widest text-brand-midnight/40">
              Password
            </label>
            <Link
      ...
      <div className="mt-8 text-center space-y-4">
        <p className="text-xs font-bold text-brand-midnight/40 uppercase tracking-widest">
          ...
          <Link href={`/register${redirect !== "/" ? `?redirect=${encodeURIComponent(redirect)}` : ""}`} className="text-brand-secondary hover:underline">
            Register
          </Link>
        </p>
      </div>
