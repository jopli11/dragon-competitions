"use client";

import type { ReactNode } from "react";
import { track, type AnalyticsEvent } from "@/lib/analytics";

export type SocialPlatform = "instagram" | "tiktok" | "facebook" | "linkedin";

type SocialLink = {
  id: SocialPlatform;
  name: string;
  href: string;
  analyticsEvent: AnalyticsEvent;
  icon: ReactNode;
};

export const SOCIAL_LINKS: SocialLink[] = [
  {
    id: "instagram",
    name: "Instagram",
    href: "https://www.instagram.com/coast.competitions",
    analyticsEvent: "social_instagram_click",
    icon: (
      <svg fill="currentColor" viewBox="0 0 24 24" className="h-full w-full">
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.774 4.919 4.851.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.075-1.664 4.703-4.919 4.85-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.775-4.919-4.851-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.075 1.664-4.704 4.919-4.85 1.265-.058 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.162 6.162 6.162 6.162-2.759 6.162-6.162-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
      </svg>
    ),
  },
  {
    id: "tiktok",
    name: "TikTok",
    href: "https://www.tiktok.com/@coastcompetitions",
    analyticsEvent: "social_tiktok_click",
    icon: (
      <svg fill="currentColor" viewBox="0 0 24 24" className="h-full w-full">
        <path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z" />
      </svg>
    ),
  },
  {
    id: "facebook",
    name: "Facebook",
    href: "https://www.facebook.com/people/Coast-Competitions/61577824716872/",
    analyticsEvent: "social_facebook_click",
    icon: (
      <svg fill="currentColor" viewBox="0 0 24 24" className="h-full w-full">
        <path d="M22.675 0H1.325C.593 0 0 .593 0 1.325v21.351C0 23.408.593 24 1.325 24H12.82v-9.294H9.692V11.08h3.128V8.413c0-3.1 1.893-4.788 4.659-4.788 1.325 0 2.464.099 2.795.143v3.24l-1.918.001c-1.504 0-1.795.715-1.795 1.763v2.313h3.587l-.467 3.626H16.561V24h6.115c.73 0 1.324-.592 1.324-1.324V1.325C24 .593 23.408 0 22.675 0z" />
      </svg>
    ),
  },
  {
    id: "linkedin",
    name: "LinkedIn",
    href: "https://www.linkedin.com/company/coast-competitions/",
    analyticsEvent: "social_linkedin_click",
    icon: (
      <svg fill="currentColor" viewBox="0 0 24 24" className="h-full w-full">
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.063 2.063 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
      </svg>
    ),
  },
];

type SocialLinksProps = {
  /** Tailwind size classes for the icon container, e.g. "h-8 w-8". */
  className?: string;
  /** Tailwind size classes for the SVG itself. Default tightly hugs the wrapper. */
  iconClassName?: string;
  /** Optional override for the wrapper class (background, border, etc.). */
  wrapperClassName?: string;
  /** Optional override for the gap between icons. */
  gapClassName?: string;
};

const DEFAULT_WRAPPER =
  "w-8 h-8 rounded-full bg-white/5 flex items-center justify-center hover:bg-brand-secondary transition-colors cursor-pointer border border-white/5";

export function SocialLinks({
  className,
  iconClassName = "w-4 h-4",
  wrapperClassName = DEFAULT_WRAPPER,
  gapClassName = "gap-4",
}: SocialLinksProps) {
  return (
    <div className={`flex ${gapClassName} ${className ?? ""}`}>
      {SOCIAL_LINKS.map((social) => (
        <a
          key={social.id}
          href={social.href}
          target="_blank"
          rel="noopener noreferrer"
          onClick={() => track(social.analyticsEvent)}
          className={wrapperClassName}
          title={social.name}
          aria-label={social.name}
        >
          <span className={`inline-flex ${iconClassName}`} aria-hidden>
            {social.icon}
          </span>
        </a>
      ))}
    </div>
  );
}
